...
def index (request):
    if 'term' in request.GET:
        ...
        school_name = []
        # print(qs.first().school_name)
        for s in qs:
            school_name.append(s.school_name)
        return JsonResponse(school_name,safe=False)
    metaDatas = SchoolData.objects.all()
    return render(request, 'index.html',{'metaDatas':metaDatas})

def thaiSchoolMap (request):
    if 'term' in request.GET:
        qs = SchoolData.objects.filter(school_name__istartswith=request.GET.get('term')).only("school_name")
        school_name = []
        for s in qs:
    ...
    #     res = SchoolData.objects.filter(school_name__iexact=request.GET['searchSchoolName'])
    return render(request, 'thaiSchoolMap.html')
...
#         q_sector = Q(sector = req_sector)
#         q_list.append(q_sector)
        
#     if req_size !='ทุกขนาด':
#         q_size = Q(summary_school_id__size = req_size)
#         q_list.append(q_size) 

#     if req_province != 'ทุกจังหวัด':
#         q_province = Q(province = req_province)
#         q_list.append(q_province)
    
#     if len(q_list) == 0:
#          q_req = Q(school_id__isnull = False)
#     else:
#         q_req = Q()
#         for q in q_list:
#             q_req = q_req & q
#     #-------------End query condition-----------------------
#     fSchoolData = SchoolData.objects.filter(q_req).order_by('-summary_school_id__total_student')[0:100]

#     fSchoolData_serializer = SchoolDataSerializer(fSchoolData.values(), many=True)
#     schoolData_json_str = json.dumps(fSchoolData_serializer.data)

#     schoolJson_list = ConvertToGeoJsonList(schoolData_json_str)
#     return JsonResponse(schoolJson_list, safe = False)
#     # return JsonResponse(fGeoData_serializer.data, safe=False)

# def ConvertToGeoJsonList(schoolData_json_str):
#     schoolData_json_list = json.loads(schoolData_json_str)
#     schoolJson_list = []
    
#     for i in range(len(schoolData_json_list)):       
#         point = {
#             "type": "Feature",
#             "geometry": {
#                 "type": "Point",
#                 "coordinates": [schoolData_json_list[i]['lng'], schoolData_json_list[i]['lat']]
#             },
#             "properties": {
#                 "name": schoolData_json_list[i]['school_name']
#             }
#         }
#         schoolJson_list.append(point)
#     return  schoolJson_list
